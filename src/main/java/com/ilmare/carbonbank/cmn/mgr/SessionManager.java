...
public class SessionManager {
	...
	public boolean isSession() {
		if (isSession(ILMARE_SESS_INFO)) {
			if (StringUtil.isNullOrEmpty(getSessInfo().getMbrId()))
				return false;
			return true;
		}
		return false;
	...
	public void sessionInit(HttpServletRequest req, SessInfo sessinfo) {
		...
		httpSession.setAttribute("mbrPwd", (StringUtil.nullStringToEmpty(sessinfo.getMbrPwd()))); // 앙호화된 패스워드
		...
		httpSession.setAttribute("loginInd", (StringUtil.nullStringToEmpty(sessinfo.getLoginInd()))); // 로그인 구분:M:회원,
																										// S:가맹점, A:회원,
																										// 가맹점 둘다 가입
	...
	public SessInfo getSession(HttpServletRequest req) {
		...
		sessinfo.setMbrPwd(StringUtil.nullStringToEmpty((String) httpSession.getAttribute("mbrPwd"))); // 암호화된 패스워드
		...
		sessinfo.setLstLgnDtm(StringUtil.nullStringToEmpty((String) httpSession.getAttribute("lstLgnDtm"))); // 최근 로그인
																												// 일시로그인
		sessinfo.setStoreId(StringUtil.nullStringToEmpty((String) httpSession.getAttribute("storeId"))); // 가입된 가맹점 ID
		sessinfo.setLoginInd(StringUtil.nullStringToEmpty((String) httpSession.getAttribute("loginInd"))); // 로그인
																											// 구분:M:회원,
																											// S:가맹점,
																											// A:회원, 가맹점
																											// 둘다 가입
	...
	public boolean isSession(HttpServletRequest req) {
		...
		if (StringUtil.nullStringToEmpty((String) httpSession.getAttribute("mbrId")).length() < 2
				&& StringUtil.nullStringToEmpty((String) httpSession.getAttribute("storeId")).length() < 2) {
			return false;
		}
		return true;
	}
